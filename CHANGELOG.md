...
### Added
- **`delete-account` (erpclaw-gl).** New action that permanently deletes an account, registered in `erpclaw-gl`'s `ACTIONS` dict and in the foundation router's `ACTION_MAP`/`DANGEROUS_ACTIONS` (requires `--user-confirmed` at the router). Takes the existing `--account-id` (required) plus the standard optional `--company-id`/`--company` — no new flags. Refuses deletion, reporting every applicable reason together in one message rather than stopping at the first, when the account has: (1) a non-zero balance (compared as `Decimal`, never string/float); (2) posted GL/journal history — any `gl_entry` row ever posted for the account, including cancelled ones, or any `journal_entry_line` whose parent `journal_entry.status` has reached `submitted`/`cancelled`/`amended`; or (3) one or more child accounts (via `parent_id`). A `journal_entry_line` still referencing the account through a **draft** journal entry is reported as its own distinct condition (clear the draft first) rather than folded into posted history, but it still blocks deletion. An empty `is_group=1` account with no balance/history/children is deletable — the group flag alone is never a blocker. A generic safety net catches any other `sqlite3.IntegrityError` from the remaining tables with a `RESTRICT` foreign key to `account` and returns one non-specific message ("account is still referenced elsewhere and cannot be deleted") — never a raw SQLite error, table, or column name. There is no `--force`/override flag, by design; the only remediation is clearing the blocking condition or using `freeze-account`/disable instead of deleting. When `--company-id`/`--company` is supplied, cross-company deletion is rejected; when omitted, no company check runs, matching the existing `freeze-account`/`unfreeze-account` precedent (not a new gap introduced here). Deliberately CLI-only in this release: no `UI.yaml` entry and no web dashboard trigger, since the erpclaw-web relay (`api/skills/executor.py`) does not currently pass `--user-confirmed` on any forwarded action. SKILL.md's General Ledger action count corrected 30 → 32 (the prior "30" label was already one stale before this addition; the true pre-existing count was 31).
